...
const TimeScroller = ({ title, data, onChange }) => {
  const { options = defaultOptions, utils, state } = useCalendar();
  const [mainState] = state;
  const hour = new Date(mainState.selectedDate).getHours();
  const minute = new Date(mainState.selectedDate).getMinutes();
  ...
  // useEffect(() => {
  //   scrollAnimatedValue = new Animated.Value(title === utils.config.hour ? hour * itemSize : minute * itemSize);
  // }, [])
  ...
  return (
    <View style={style.row} onLayout={changeItemWidth}>
      ...
      <AnimatedFlatList
        ref={flatListRef}
        // initialScrollIndex={title === utils.config.hour ? hour : minute}
        pagingEnabled
